...
如果你想看 flash 中的 record / sector / checkpoint 具体长什么样，请再看 [fee_diag_test.md](./fee_diag_test.md)。
...
板级或驱动层要实现的是 `fee_flash_drv.h` 中这些符号：
...
- 当前 `fee_port.c` 设计上可自带默认 `weak` RAM mock 实现
